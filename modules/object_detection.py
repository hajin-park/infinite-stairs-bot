...
class ObjectDetection:
    ...
    def search_segmented_layer(self,
        ...
        left_area_dimensions = [
            top, bottom, self.vertical_layers[x_pos-2], self.vertical_layers[x_pos-1]]
        if self.search_area(left_area_dimensions, layer):
            return

        #   Search right screenshot segment if necessary
        right_area_dimensions = [
            top, bottom, self.vertical_layers[x_pos], self.vertical_layers[x_pos+1]]
        if self.search_area(right_area_dimensions, layer):
            return
    ...
    def main(self) -> None:
        ...
        self.search_segmented_layer(
            self.horizontal_layers[11], self.horizontal_layers[12], 4, 12)
