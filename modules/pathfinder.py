class PathFinder:
    ...
    def fill_edge(self, stair):
        ...
        if (stair.x not in (1, 7)): return True
    ...
    def fill_zero(self, delta_x, stair, next_stair, next_is_last):
        if (delta_x not in (-1, 1)): return True

        if next_is_last: self.fill_last(stair, next_stair)

    def fill_one(self, delta_x, stair, next_stair, next_is_last):
        if (delta_x not in (-2, 2, 0)): return True

        #   current stair is next to next stair
        elif (delta_x in (-2, 2)): self.stairs_connected.append([stair.x-(delta_x/2), stair.y-1])

        #   current stair is under next stair
        elif (stair.x in (1, 5)): self.stairs_connected.append([stair.x-1, stair.y-1])

        #   current stair is under next stair
        elif (stair.x in (3, 7)): self.stairs_connected.append([stair.x+1, stair.y-1])
    ...
    def fill_two(self, delta_x, stair, next_stair, next_is_last):
        if (delta_x not in (-3, 3)): return True
    ...
    def fill_path(self) -> None:
        ...
        if (self.stairs_detected[0].y != 12 or self.stairs_detected[0].x not in (3, 5)): return
        ...
        for (i, stair) in enumerate(self.stairs_detected[:-1], start=1):
            ...
            if (delta_y != 1 and stair.y < 7): return

            elif (delta_y == 1):
                if self.fill_zero(delta_x, stair, next_stair, next_is_last): return

            elif (delta_y == 2):
                if self.fill_one(delta_x, stair, next_stair, next_is_last): return

            elif (delta_y == 3):
                if self.fill_two(delta_x, stair, next_stair, next_is_last): return

            #   end pathfinding if there are more than two consecutive empty layers
            else: return self.fill_edge(stair)
    ...
    def next_action(self, current_stair: float, next_stair: int) -> str:
        ...
        if (button == 'left'): self.left = not self.left
