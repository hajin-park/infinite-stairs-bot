...
class WindowCapture:
    ...
    def __init__(self):
        ...
        window_location = win32.ClientToScreen(window_handle, (client_rect[1], client_rect[0]))
        self.win_rect   = {
        ...
        self.sct_y_loc         = int(self.win_rect['top']+self.win_rect['height']*0.11111)-30
        self.horizontal_layers = [y for y in range(0, self.sct_rect['height']+1, 47)]
        self.vertical_layers   = [0]+[x for x in range(66, self.sct_rect['width'], 87)]+[self.sct_rect['width']]
        self.lock              = Lock()
    ...
    def draw_rectangles(self):
        ...
        for (x, y) in self.rects:
            if (x > 0  and x < 8): 
                cv2.rectangle(
                    self.temp_sct, 
                    (self.vertical_layers[int(x-1)], self.horizontal_layers[int(y-1)]), 
                    (self.vertical_layers[int(x)], self.horizontal_layers[int(y)]), 
                    (0,0,255), 
                    4
                )
